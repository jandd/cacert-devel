...
sub RevokeX509 {
    ...
    my $current_hash = sha1_hex(readfile("revoke-root$root.crl"));
    ...
    if (open OUT, ">$wid/request.crt") {
        ...
        my $do = `$opensslbin ca $hashes{$hash} -config $openssl_config -key test -batch -revoke $wid/request.crt > /dev/null 2>&1`;
        $do = `$opensslbin ca $hashes{$hash} -config $openssl_config -key test -batch -gencrl -crldays 7 -crlexts crl_ext -out $wid/cacert-revoke.crl > /dev/null 2>&1`;
