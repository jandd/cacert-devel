...
my %revoke_file = (2 => "$crl_dir/class3-revoke.crl", 1 => "$crl_dir/revoke.crl");
...
my Device::SerialPort $PortObj = tie(*SER, 'Device::SerialPort', "serial.conf") || LogErrorAndDie(
...
sub SendHandshaked($) {
    ...
    if ($length && $data eq "\x10") {
        ...
        while ($try_again == 1) {
            ...
            LogErrorAndDie("Packet receipt was not confirmed in 5 seconds. Connection lost!\n") if (!scalar($sel->can_read(5)));
            ...
            if ($length && $data eq "\x10") {
                ...
                LogErrorAndDie(sprintf("I cannot send! %d %s\n", $length, unpack("C", $data)));
            }
        }
...
sub calculateDays($) {
    if ($_[0]) {
        my @sum = $dbh->selectrow_array(sprintf(
                "SELECT SUM(points) AS total FROM notary WHERE `to` ='%s' AND deleted = 0 GROUP BY `to`", $_[0]));
        SysLog("Summe: $sum[0]\n") if ($debug >= 1);
...
sub OpenPGPextractExpiryDate($) {
    ...
    open(RGPG, $gpg_binary . ' -vv ' . $_[0] . ' 2>&1 |') or LogErrorAndDie('Can\'t start GnuPG($gpgbin): ' . $! . "\n");
    open(OUT, '> infogpg.txt') or LogErrorAndDie('Can\'t open output file: infogpg.txt: ' . $!);
    $/ = "\n";
    while (<RGPG>) {
        print OUT $_;
        unless ($r) {
...
sub sendmail($$$$$$$) {
    ...
    $new_subject =~ s/\n*$//;
    print $smtp trim($subject) =~ m/[^a-zA-Z0-9 ,.\[\]\/-]/ ? "Subject: =?utf-8?B?$new_subject?=\r\n" : "Subject: $subject\r\n";
...
sub HandleCerts($$) {
    ...
    my $query = "SELECT * FROM $table WHERE crt_name='' AND csr_name!='' AND warning<3";
    SysLog("$query\n") if ($debug >= 1);
    ...
    while (my $row_data = $sth->fetchrow_hashref()) {
        ...
        my $csr_name = sprintf("%s/%s/%s/%s-%d.csr", $csr_dir, $prefix, $short, $prefix, $row{'id'});
        my $crt_name = sprintf("%s/%s/%s/%s-%d.crt", $crt_dir, $prefix, $short, $prefix, $row{'id'});
        ...
        if ($server == 1) {
            #Weird SQL structure ...
            my @sqlres = $dbh->selectrow_array(sprintf("SELECT memid FROM domains WHERE id='%d'", int($row{'domid'})));
            $row{'memid'} = $sqlres[0];
        ...
        if (open(IN, "<$csr_name")) {
            ...
            if (length($crt)) {
                if ($crt =~ m/^-----BEGIN CERTIFICATE-----/) {
                    ...
                    print OUT $crt;
                    close OUT;
                    system "$openssl_binary x509 -in $crt_name.der -inform der -out $crt_name";
                }
            }
            else {
        ...
        if (-s $crt_name) {
            ...
            SysLog("Could not find the issued certificate. $crt_name " . $row{"id"} . "\n");
...
sub DoCRL($$) {
    ...
    if (length($crl)) {
        if ($crl =~ m/^-----BEGIN X509 CRL-----/) {
            ...
            print OUT $crl;
            close OUT;
            system "$openssl_binary crl -in $crl_name.pem -outform der -out $crl_name.tmp";
        }
        else {
            open OUT, ">$crl_name.patch";
            print OUT $crl;
            close OUT;
            my $res = system "xdelta patch $crl_name.patch $crl_name $crl_name.tmp";
        ...
        my $res = `openssl crl -verify -in $crl_name.tmp -inform der -noout 2>&1`;
        SysLog("verify: $res\n");
        if ($res =~ m/verify OK/) {
...
sub RefreshCRLs() {
    foreach my $rootcert (keys %revoke_file) {
        if (!CRLuptodate($revoke_file{$rootcert})) {
            ...
            my $crl = Request($ver, 2, 1, $rootcert - 1, 0, 0, 365, 0, "", "", $revoke_hash);
...
sub HandleGPG() {
    ...
    while (my $row_data = $sth->fetchrow_hashref()) {
        ...
        if (-s $crt_name) {
            ...
            my $query = sprintf("UPDATE gpg SET crt='%s', issued=NOW(), expire='%s' WHERE id='%d'",
